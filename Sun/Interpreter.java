 ...
 @Override
  ...
  public Object visitThisExpr(Expr.This expr) {
    ...
    public Object visitUnaryExpr(Expr.Unary expr) {
        ...
        // we evaluate the operand expression.
        // Then we apply the unary operator itself to the result of that.
        ...
        switch (expr.operator.type) {
            ...
            case MINUS:
                // -, which negates the result of the subexpression.
                // The subexpression must be a number. Since we don’t statically know that in
                // Java, we cast it before performing the operation.
                return -(double) right;
        }
...
/*we look up the resolved distance in the map. Remember that we resolved only local variables. Globals are treated specially and don’t end up in the map (hence the name locals). So, if we don’t find a distance in the map, it must be global. In that case, we look it up, dynamically, directly in the global environment. That throws a runtime error if the variable isn’t defined. */
    ...
    // explaination in the page of Evaluting expr 5 heading
    ...
    // This is one of those corners where the details of how we represent Lox
    // objects in terms of Java matter.
    // We need to correctly implement Lox’s notion of equality, which may be
    ...
    // method which simply sends the expression back into the interpreter’s visitor
    // implementation:
 ...
 @Override
  ...
  public Object visitAssignExpr(Expr.Assign expr) {
    ...
    public Object visitBinaryExpr(Expr.Binary expr) {
        Object left = evaluate(expr.left);
        Object right = evaluate(expr.right);
        // The main difference from the unary negation operator is that we have two
        ...
        switch (expr.operator.type) {
            ...
            // To handle that, we don’t just assume the operands are a certain type and cast
            // them,
